...
class CurrencyDropdownAdapter(
    ...
    init {
        ...
        if (recentCurrencies.isNotEmpty()) {
            itemList.add(CurrencyItem.Header(R.string.recent_currencies))
            position++
            ...
            itemList.add(CurrencyItem.Header(R.string.all_currencies))
            position++
        }
        ...
        return when (item) {
            ...
            is CurrencyItem.Header -> {
                val view = convertView?.takeIf { it.tag == TYPE_HEADER }
                    ?: LayoutInflater.from(context)
                        .inflate(R.layout.item_currency_divider, parent, false)
